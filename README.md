...
[OIST、デジタル画像の「視覚的な鮮明さ」を定量的に評価する手法を提案](https://news.mynavi.jp/techplus/article/20241217-3088143/)  
...
## DeltaS_M の説明
指標 **DeltaS_M** は、画像の構造的安定性を定量化するための画期的な指標です。この指標は、オリジナル画像と隣接するピクセルをランダムに反転させた「溶解」画像を比較することで計算されます。完全に均一またはランダムな画像には0が割り当てられ、識別可能なパターンを持つ画像には有限の値が割り当てられます。この計算過程では、元の画像の色多様性を尊重し、画像圧縮や色量子化にも耐性があります。**DeltaS_M** は、テクスチャ画像の評価や物理システム（例：Pottsモデル）の構造遷移の特定に有効であり、デジタルアートの色彩理論にも一貫性を持っています。さらに、この指標は、画像の色情報をスピン構造に類似する状態変数として扱い、明確な構造を持つ状態への遷移を分析することができます。これにより、科学および創造的な分野において意思決定を向上させる実用的なツールとして機能します。
